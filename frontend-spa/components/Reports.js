const Reports = Vue.defineComponent({
  ...
  <AdminLayout title="Semua Laporan" subtitle="Daftar seluruh laporan masyarakat beserta status penanganannya.">
    ...
    <div class="bg-white rounded-b-2xl border border-[#E2E8F0] shadow-sm overflow-hidden mb-8">
      ...
      <!-- Pagination Footer -->
      <div v-if="filteredReports.length > 0" class="px-6 py-4 border-t border-[#E5E7EB] bg-white flex flex-col xl:flex-row gap-4 items-center justify-between">
        <!-- Left: Info -->
        <div class="text-[14px] font-medium text-[#64748B]">
          Menampilkan <span class="font-semibold text-slate-900">{{ startIndex }}</span>–<span class="font-semibold text-slate-900">{{ endIndex }}</span> dari <span class="font-semibold text-slate-900">{{ filteredReports.length }}</span> laporan
        </div>
